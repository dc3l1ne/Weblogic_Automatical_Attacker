...
class ver9000():
	...
	def get_path(self,url,headers,warname):
		try:
			path_exp=warname+' [^ -~]*[ -~]*'
	...
	def test(self,url,depolyname,testpage):
		...
		if test_data.status_code == 200:
			...
			f=open('good.txt', 'a')
			if self.system == 1:
				f.write(test_url+'\tWebLogic 9\t\t'+self.domain_name+'\t\t'+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
			else:
				f.write(test_url+'\tWebLogic 9\t\t'+self.domain_name+'\t\t'+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
		else:
			print 'Test Failed!'
			f=open('bad.txt', 'a')
			if self.system == 1:
				f.write('Test Failed:'+test_url+'\tWebLogic 9\t\t'+self.domain_name+'\t\t'+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
			else:
				f.write('Test Failed:'+test_url+'\tWebLogic 9\t'+self.domain_name+'\t\t'+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
