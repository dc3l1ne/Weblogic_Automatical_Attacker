...
class ver10330():
	...
	def test(self,url,depolyname,testpage,verse):
		...
		if test_data.status_code == 200:
			...
			f=open('good.txt', 'a')
			if self.system == 1:
				f.write(test_url+'\tWebLogic '+verse.group()+'\t'+self.domain_name+'\t\t'+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
			else:
				f.write(test_url+'\tWebLogic '+verse.group()+'\t'+self.domain_name+'\t\t'+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
		else:
			print 'Test Failed!'
			f=open('bad.txt', 'a')
			if self.system == 1:
				f.write('Test Failed:'+test_url+'\tWebLogic '+verse.group()+'\t'+self.domain_name+'\t\t'+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
			else:
				f.write('Test Failed:'+test_url+'\tWebLogic '+verse.group()+'\t'+self.domain_name+'\t\t'+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
