...
class ver81():
	...
	def get_path(self,url,headers,count):
		ctest=0
		while True:
			if ctest == count:
				return False
	...
	def test(self,url,testpage,depolyname,count):
		ctest=0
			...
			if ctest == count:
				...
				try:
					...
					if test_data.status_code == 200:
						...
						f=open('good.txt', 'a')
						if self.system == 1:
							f.write(test_url+t2+'WebLogic Server 8.1\t'+self.domain_name+t+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
							f.close()
							return
						else:
							f.write(test_url+t2+'WebLogic Server 8.1\t'+self.domain_name+t+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
							f.close()
							return
					else:
						print 'Test Failed!'
						f=open('bad.txt', 'a')
						if self.system == 1:
							f.write('Test Failed:'+test_url+t2+'WebLogic Server 8.1\t'+self.domain_name+t+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
							f.close()
							return
						else:
							f.write('Test Failed:'+test_url+t2+'WebLogic Server 8.1\t'+self.domain_name+t+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
							f.close()
							return
				except:
					print 'test failed!\n'
