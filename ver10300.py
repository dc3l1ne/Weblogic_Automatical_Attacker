...
class ver10300():
	...
	def get_cookie(self,url,headers,usr,pwd):
		try:
			data = {'j_username':usr,'j_password':pwd}
	...
	def check_active(self,url,headers):
		try:
			active_data=requests.get(url+'/console.portal?_nfpb=true&_pageLabel=AppDeploymentsControlPage',cookies=self.cookies,headers=headers,timeout=10)
			if active_data.text.count('Future changes will automatically be activated') != 0:
				print 'Automatical active is On!\n'
				return None
			elif active_data.text.count('add or delete items in this domain') != 0 or active_data.text.count('No pending changes exist'):
	...
	def test(self,url,depolyname,testpage):
		...
		if test_data.status_code == 200:
			...
			f=open('good.txt', 'a')
			if self.system == 1:
				f.write(test_url+'\tWebLogic 10.3.0.0\t'+self.domain_name+'\t\t'+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
			else:
				f.write(test_url+'\tWebLogic 10.3.0.0\t'+self.domain_name+'\t\t'+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
		else:
			print 'Test Failed!'
			f=open('bad.txt', 'a')
			if self.system == 1:
				f.write('Test Failed:'+test_url+'\tWebLogic 10.3.0.0\t'+self.domain_name+'\t\t'+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
			else:
				f.write('Test Failed:'+test_url+'\tWebLogic 10.3.0.0\t'+self.domain_name+'\t\t'+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
				f.close()
