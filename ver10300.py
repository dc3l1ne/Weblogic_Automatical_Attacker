...
class ver10300():
	...
	def check_active(self,url,headers,count):
		ctest=0
		while True:
			if ctest == count:
				return False
		...
		while True:
			if ctest == count:
				...
				if test_data.status_code == 200:
					...
					f=open('good.txt', 'a')
					if self.system == 1:
						f.write(test_url+t2+'WebLogic 10.3.0.0\t'+self.domain_name+t+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
						f.close()
						return
					else:
						f.write(test_url+t2+'WebLogic 10.3.0.0\t'+self.domain_name+t+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
						f.close()
						return
				else:
					print 'Test Failed!'
					f=open('bad.txt', 'a')
					if self.system == 1:
						f.write('Test Failed:'+test_url+t2+'WebLogic 10.3.0.0\t'+self.domain_name+t+'Windows\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
						f.close()
						return
					else:
						f.write('Test Failed:'+test_url+t2+'WebLogic 10.3.0.0\t'+self.domain_name+t+'Linux\t'+time.strftime("%Y-%m-%d[%H.%M.%S]")+'\n')
						f.close()
						return
